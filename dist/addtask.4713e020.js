...
const progressTypeAdditionalInfoDiv = document.querySelector(".progress-type__additional-info");
...
inputProgressType.addEventListener("change", (event)=>{
    ...
    if (progressType === "time") {
        ...
        progressTypeAdditionalInfoDiv.insertAdjacentHTML("beforeend", `<label for="time">Choose a time: </label>
      ...
      `);
        ...
        document.querySelector(".graph-type").removeAttribute("hidden");
        progressTypeAdditionalInfoDiv.insertAdjacentHTML("beforeend", `<div class="number-range" style="display: flex; gap: 20px;">
        <div>
      ...
      `);
        ...
        numberRange.addEventListener("change", (event)=>{
            ...
            if (numberType === "start-number") progressNumberStart = Number(event.target.value);
            else progressNumberEnd = Number(event.target.value);
        });
    } else if (progressType === "percentage") {
        document.querySelector(".graph-type").removeAttribute("hidden");
        progressTypeAdditionalInfoDiv.insertAdjacentHTML("beforeend", `<label for="target-percentage">Choose target percentage: </label>
       <input id="target-percentage" class="target-percentage__value" style="width:80px; padding: 5px 10px;" type="number" required />
      `);
        document.querySelector(".target-percentage__value").addEventListener("change", (event)=>{
            if (event.target.value < 0) event.target.value = "0";
        ...
        progressTypeAdditionalInfoDiv.insertAdjacentHTML("beforeend", `<label for="target-no-of-days">Select target: </label>
      ...
      `);
        ...
        document.querySelector(".target-no-of-days__value").addEventListener("change", (event)=>{
...
btnAddTask.addEventListener("click", ()=>{
    ...
    progressType = inputProgressType.value;
    if (progressType === "default") alert("Please select a progress type");
    if (graphType === "default") alert("Please select a graph type");
    if (progressType === "number" && (progressNumberStart === undefined || progressNumberEnd === undefined)) alert("Please fill the number range input");
    else if (progressType === "number" && progressNumberStart > progressNumberEnd) alert("Please select appropriate number range with start number less than end number");
...
const validateProgressInfo = (progressType)=>{
    switch(progressType){
        case "percentage":
            if (isNaN(targetPercentage)) return false;
            return true;
        case "number":
            if (isNaN(progressNumberStart) || isNaN(progressNumberEnd)) return false;
            ...
            return true;
        case "dichotomous":
            if (dichotomousTypeTarget === "default") return false;
            return true;
        case "time":
...
const renderAcceptedDeviationHTML = (progressType, graphType)=>{
    switch(progressType){
        case "percentage":
            {
                ...
                console.log(`Max: ${max}`);
                inputAcceptedDeviationDiv.insertAdjacentHTML("beforeend", `<label for="accepted-deviation__id">Accepted deviation: </label>
        ...
        case "dichotomous":
            ...
            inputAcceptedDeviationDiv.insertAdjacentHTML("beforeend", `<label for="accepted-deviation__id">Accepted deviation: </label>
        <input id="accepted-deviation__id" style="width:80px; padding: 5px 10px;" class="accepted-deviation__value" type="number" min=0 max=${getProgressEachDayObj(priority).length} />`);
        case "time":
    }
};
addTaskContainer.addEventListener("change", (event)=>{
    console.log(inputGraphTypeDiv.hasAttribute("hidden"));
    console.log(!inputGraphTypeDiv.hasAttribute("hidden") && graphType !== "default");
    if (progressType !== "default" && (inputGraphTypeDiv.hasAttribute("hidden") || !inputGraphTypeDiv.hasAttribute("hidden") && graphType !== "default") && validateProgressInfo(progressType)) {
...
},{"9034c5b7b162606":"jwcsj","chart.js":"ipU8D","tippy.js":"ccpCS","tippy.js/dist/tippy.css":"2hEyg","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"ccpCS":[function(require,module,exports) {
/**!
* tippy.js v6.3.7
* (c) 2017-2021 atomiks
* MIT License
